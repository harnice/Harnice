...
for required_channel in disconnect_channel_map():
    ...
    for candidate in available_candidates:
        ...
        if required_ch_attributes.get(
            "A-side_device_channel_type_id"
        ) in icd.compatible_channel_types(candidate.get("B-port_channel_type")):
            map_mode = 3
        ...
        if extra_verbose:
            print(
                f"          Channel type of A-side device {required_ch_attributes.get('A-side_device_channel_type_id')} is not found in compatibles of channel type of B-port of disconnect {icd.compatible_channel_types(candidate.get('B-port_channel_type'))}"
            )

        if required_ch_attributes.get(
            "B-side_device_channel_type_id"
        ) in icd.compatible_channel_types(candidate.get("A-port_channel_type")):
            map_mode = 4
        ...
        if extra_verbose:
            print(
                f"          Channel type of B-side device {required_ch_attributes.get('B-side_device_channel_type_id')} is not found in compatibles of channel type of A-port of disconnect {icd.compatible_channel_types(candidate.get('A-port_channel_type'))}"
            )

        if candidate.get("A-port_channel_type") in icd.compatible_channel_types(
        ...
        if extra_verbose:
            print(
                f"          Channel type of A-port of disconnect {candidate.get('A-port_channel_type')} is not found in compatibles of channel type of B-side device {icd.compatible_channel_types(required_ch_attributes.get('B-side_device_channel_type_id'))}"
            )

        if candidate.get("B-port_channel_type") in icd.compatible_channel_types(
        ...
        if extra_verbose:
            print(
                f"          Channel type of B-port of disconnect {candidate.get('B-port_channel_type')} is not found in compatibles of channel type of A-side device {icd.compatible_channel_types(required_ch_attributes.get('A-side_device_channel_type_id'))}"
            )

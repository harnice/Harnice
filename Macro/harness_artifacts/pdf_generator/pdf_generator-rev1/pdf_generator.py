...
if not os.path.exists(page_setup_path) or os.path.getsize(page_setup_path) == 0:
    ...
    try:
        ...
        raise ValueError(f"[ERROR] Invalid JSON in page setup file: {page_setup_path}\n{e}")

# --- STRUCTURE VALIDATION ---
if not isinstance(page_data, dict):
    raise TypeError(f"[ERROR] Page setup must be a dict, got {type(page_data).__name__}")
...
if not isinstance(page_data["pages"], list):
    raise TypeError(f"[ERROR] 'pages' must be a list, got {type(page_data['pages']).__name__}")

for i, page in enumerate(page_data["pages"], start=1):
    if not isinstance(page, dict):
        raise TypeError(f"[ERROR] Page {i} in 'pages' list must be a dict, got {type(page).__name__}")
    if "name" not in page:
...
# ============================================
    ...
    os.makedirs(fileio.dirpath(f"macro-{artifact_id}-{page_name}", structure_dict=file_structure(page_name=page_name)), exist_ok=True)
    shutil.copy(
        fileio.path("project titleblock attributes", structure_dict=file_structure(page_name=page_name)),
        fileio.path("macro titleblock attributes", structure_dict=file_structure(page_name=page_name))
    )
    shutil.copy(
...
# ============================================
    with open(fileio.path("macro titleblock attributes", structure_dict=file_structure(page_name=page_name)), "r", encoding="utf-8") as f:
    ...
    svg_utils.find_and_replace_svg_group(
        fileio.path("tblock with text replacements", structure_dict=file_structure(page_counter=page_counter, page_name=page_name)),
        fileio.path("macro titleblock drawing", structure_dict=file_structure(page_counter=page_counter, page_name=page_name)),
        "tblock",
        "tblock",
    )
    ...
    with open(fileio.path("tblock with text replacements", structure_dict=file_structure(page_counter=page_counter, page_name=page_name)), "r", encoding="utf-8") as f:
        svg = f.read()
    ...
    for old, new in text_map.items():
        ...
        if new == "autosheet":
            page_names = [
                p.get("name") for p in page_data.get("pages", [])
            ]
...
for page in page_data.get("pages", []):
    ...
    if not os.path.exists(fileio.path("user editable page svg", structure_dict=file_structure(page_counter=page_counter, page_name=page_name))):
        with open(fileio.path("user editable page svg", structure_dict=file_structure(page_counter=page_counter, page_name=page_name)), "w", encoding="utf-8") as f:
            f.write(
                # <?xml version="1.0" encoding="UTF-8" standalone="no"?>
                f"""
    ...
    svg_utils.find_and_replace_svg_group(
        fileio.path("user editable page svg", structure_dict=file_structure(page_counter=page_counter, page_name=page_name)), fileio.path("master contents svg", structure_dict=file_structure()), "svg-master", "svg-master"
    )

    # replace the titleblock
    svg_utils.find_and_replace_svg_group(
        fileio.path("user editable page svg", structure_dict=file_structure(page_counter=page_counter, page_name=page_name)),
        fileio.path("tblock with text replacements", structure_dict=file_structure(page_counter=page_counter, page_name=page_name)),
...
# Merge all PDFs
subprocess.run(["pdfunite"] + temp_pdfs + [fileio.path("output pdf", structure_dict=file_structure())], check=True)

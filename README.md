...
There are two important functions that Harnice is currently prepared to handle: "Systems" and "Harnesses". A "system" is a collection of "boxes" that are connected by "harnesses", where "boxes" are any physical device that has connectors on it, and a "harness" is a collection of cables, conductors, connectors, etc, that are used to connect those box connectors together.

# Electrical systems
Systems are defined using PCB schematic software like KiCad or Altium, where a KiCad-component represents a box, and KiCad-nets represents a harness. If you lay out your system definition in this way, you can export a netlist, which represents every connector on every box that is part of the same harness.

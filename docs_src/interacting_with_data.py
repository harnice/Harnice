...
def main():
    ...
    md.append(docs_functions.print_function_docs(channel_map.already_mapped_set_append, module_prefix))
    md.append(docs_functions.print_function_docs(channel_map.already_mapped_set, module_prefix))
    md.append(docs_functions.print_function_docs(channel_map.already_mapped, module_prefix))

    path = docs_functions.harnice_dir() / "docs" / "interacting_with_data" / "_channel_maps.md"
    ...
    path = docs_functions.harnice_dir() / "docs" / "interacting_with_data" / "_circuits_lists.md"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("".join(md), encoding="utf-8")
    ...
    md.append(docs_functions.print_function_docs(disconnect_map.already_assigned_channels_through_disconnects_set_append, module_prefix))
    md.append(docs_functions.print_function_docs(disconnect_map.already_assigned_disconnects_set_append, module_prefix))
    md.append(docs_functions.print_function_docs(disconnect_map.already_assigned_channels_through_disconnects_set, module_prefix))
    md.append(docs_functions.print_function_docs(disconnect_map.already_assigned_disconnects_set, module_prefix))
    md.append(docs_functions.print_function_docs(disconnect_map.channel_is_already_assigned_through_disconnect, module_prefix))
    md.append(docs_functions.print_function_docs(disconnect_map.disconnect_is_already_assigned, module_prefix))
    md.append(docs_functions.print_function_docs(disconnect_map.ensure_requirements_met, module_prefix))

    path = docs_functions.harnice_dir() / "docs" / "interacting_with_data" / "_disconnect_maps.md"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("".join(md), encoding="utf-8")
    ...
    path = docs_functions.harnice_dir() / "docs" / "interacting_with_data" / "_formboard_graphs.md"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("".join(md), encoding="utf-8")
    ...
    md.append(docs_functions.commands_header(module_prefix))
    md.append(docs_functions.print_function_docs(instances_list.new_instance, module_prefix))
    md.append(docs_functions.print_function_docs(instances_list.modify, module_prefix))
    md.append(docs_functions.print_function_docs(instances_list.remove_instance, module_prefix))
    md.append(docs_functions.print_function_docs(instances_list.new, module_prefix))
    md.append(docs_functions.print_function_docs(instances_list.assign_bom_line_numbers, module_prefix))
    md.append(docs_functions.print_function_docs(instances_list.attribute_of, module_prefix))
    md.append(docs_functions.print_function_docs(instances_list.instance_in_connector_group_with_item_type, module_prefix))
    md.append(docs_functions.print_function_docs(instances_list.list_of_uniques, module_prefix))

    path = docs_functions.harnice_dir() / "docs" / "interacting_with_data" / "_instances_lists.md"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("".join(md), encoding="utf-8")
    ...
    path = docs_functions.harnice_dir() / "docs" / "interacting_with_data" / "_library_history.md"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("".join(md), encoding="utf-8")
    ...
    path = docs_functions.harnice_dir() / "docs" / "interacting_with_data" / "_netlists.md"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("".join(md), encoding="utf-8")
    ...
    """]
    md.append(docs_functions.commands_header(module_prefix))
    md.append(docs_functions.print_function_docs(post_harness_instances_list.rebuild, module_prefix))
    md.append(docs_functions.print_function_docs(post_harness_instances_list.push, module_prefix))
    path = docs_functions.harnice_dir() / "docs" / "interacting_with_data" / "_post_harness_instances_lists.md"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("".join(md), encoding="utf-8")
    ...
    md.append(docs_functions.print_function_docs(rev_history.initial_release_exists, module_prefix))
    md.append(docs_functions.print_function_docs(rev_history.initial_release_desc, module_prefix))
    md.append(docs_functions.print_function_docs(rev_history.update_datemodified, module_prefix))
    ...
    path = docs_functions.harnice_dir() / "docs" / "interacting_with_data" / "_revision_history_lists.md"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("".join(md), encoding="utf-8")
    ...
    md.append("\n\n---\n")
    md.append(docs_functions.commands_header(module_prefix))
    md.append(docs_functions.print_function_docs(signals_list.set_list_type, module_prefix))
    ...
    md.append(docs_functions.print_function_docs(signals_list.cavity_of_signal, module_prefix))
    md.append(docs_functions.print_function_docs(signals_list.connector_name_of_channel, module_prefix))
    md.append("---\n")
    path = docs_functions.harnice_dir() / "docs" / "interacting_with_data" / "_signals_lists.md"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("".join(md), encoding="utf-8")
    ...
    md.append(docs_functions.print_function_docs(manifest.update_upstream, module_prefix))
    md.append(docs_functions.print_function_docs(manifest.new, module_prefix))
    md.append(docs_functions.print_function_docs(manifest.update_upstream, module_prefix))
    path = docs_functions.harnice_dir() / "docs" / "interacting_with_data" / "_system_manifests.md"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("".join(md), encoding="utf-8")
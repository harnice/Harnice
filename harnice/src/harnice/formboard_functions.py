...
def generate_segments_precheck():
    ...
    if os.path.exists(file.path("formboard graph definition")):
        ...
        length_has_null = field_contains_null(file.path("formboard graph definition"), "length")
        length_has_numbers = field_contains_numbers(file.path("formboard graph definition"), "length")
        angle_has_null = field_contains_null(file.path("formboard graph definition"), "angle")
        angle_has_numbers = field_contains_numbers(file.path("formboard graph definition"), "angle")
...
def generate_segments():
    ...
    connectors = []
    with open(file.path("instances list"), mode='r') as tsv_file:
    ...
    with open(file.path("formboard graph definition"), "w") as json_file:
        json.dump(data, json_file, indent=4)
        print(f"from {basename(__file__)} > {currentframe().f_code.co_name}: Graph definition saved to {file.path("formboard graph definition")}")
...
def generate_node_coordinates():
    ...
    with open(file.path("formboard node locations inches"), "w") as file:
    ...
    with open(file.path("formboard node locations px"), "w") as file:
        json.dump(node_coordinates_px, file, indent=4)

    # Write the segment "to", "from", and "center" data to the file
    with open(file.path("formboard segment to from center"), "w") as file:
        json.dump(segment_to_from_data, file, indent=4)

    print(f"from {basename(__file__)} > {currentframe().f_code.co_name}: Node coordinates written to {file.path("formboard node locations inches")}")
    print(f"from {basename(__file__)} > {currentframe().f_code.co_name}: Node coordinates written to {file.path("formboard node locations px")}")
    print(f"from {basename(__file__)} > {currentframe().f_code.co_name}: Segment to/from/center data written to {file.path("formboard segment to from center")}")

def visualize_formboard_graph():
    node_file_path = file.path("formboard node locations px")
    output_file_path = file.path("formboard graph definition svg")

    # Read the segment and node data
    try:
        with open(file.path("formboard graph definition"), "r") as segment_file:
...
def map_connections_to_graph():
    ...
    output_path = file.path("connections to graph")

    # Read the wirelist TSV file
    try:
        with open(file.path("wirelist nolengths"), 'r') as file:
